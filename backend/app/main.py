...
# Simple heuristic: if contains Malayalam block characters -> 'mal', otherwise 'eng'
...
# Always return a string (not dict), and prefer extracting flightNumber, token, secret, city
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    try:
                        ...
                        # Not JSON — return text (trimmed)
                        return resp.text.strip()
            except Exception as e:
                print(f"Error fetching {url}: {e}")
                continue
    # If none found, return original text unchanged
...
async def hackrx_run(req: HackRxRequest):
    ...
    # First pass: try to load / extract clauses and detect direct API clauses early
    for url in doc_urls:
        try:
            ...
            # Scan extracted clauses for direct API endpoints (e.g., hackrx.in) and attempt fetch
            ...
            if flight_number_result:
                # immediate return with the flight number (document contained direct API)
                return {"answers": [flight_number_result]}

            # If doc URL is in whitelist (non-document JSON endpoints), fetch directly
